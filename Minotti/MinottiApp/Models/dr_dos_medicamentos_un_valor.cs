...
namespace Minotti
{
    // Migrado desde PowerBuilder DataWindow: dr_dos_medicamentos_un_valor.srd
    // Mantiene el nombre original del objeto DataWindow.
    public class dr_dos_medicamentos_un_valor
    {
...
WHERE ((cap.capitulo = :capitulo) OR
( 99 = :capitulo))      AND 
cap.capitulo = cm.capitulo      AND 
cap.rubrica = cm.rubrica      AND
cm.medicamento IN (:medicamento, :medicamento2)      AND 
cm.valor IN (:valor, :valor2) 
UNION  
...
AND cap.subrubrica03 = cm.subrubrica_padre      AND 
...
WHERE ((cap.capitulo = :capitulo) OR 
( 99 = :capitulo))      AND 
cap.subrubrica05 = cm.subrubrica_padre  
AND cap.subrubrica06 = cm.subrubrica_hija      AND
cm.medicamento IN (:medicamento, :medicamento2)      AND 
cm.valor IN (:valor, :valor2) 
...
cm.valor   
FROM capitulaciones_matriz cap           , subrubricacion_med cm    
WHERE ((cap.capitulo = :capitulo) OR ( 99 = :capitulo))      AND
...
cap.subrubrica09 = cm.subrubrica_hija      AND 
cm.medicamento IN (:medicamento, :medicamento2)      AND
cm.valor IN (:valor, :valor2)
UNION  
SELECT cap.capitulo_nombre,      
