...
namespace Minotti
{
    // Migrado desde PowerBuilder DataWindow: dr_global_subrubricas.srd
    // Mantiene el nombre original del objeto DataWindow.
    public class dr_global_subrubricas
    {
        ...
        public const string Sql = @"SELECT capitulos.nombre,     
                                            ...
                                            WHERE subrubricas.nombre like :campo        AND
                                            rubricas.rubrica = capitulaciones.rubrica    AND
                                            capitulos.capitulo = capitulaciones.capitulo     AND
                                            rubricaciones.rubrica = rubricas.rubrica    AND 
                                            rubricaciones.subrubrica = subrubricaciones.subrubrica_padre     AND 
                                            ( (subrubricaciones.subrubrica_padre = subrubricas.subrubrica) OR 
                                            (subrubricaciones.subrubrica_hija = subrubricas.subrubrica) )";
