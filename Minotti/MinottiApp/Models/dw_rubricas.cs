...
namespace Minotti
{
    // Migrado desde PowerBuilder DataWindow: dw_rubricas.srd
    // Mantiene el nombre original del objeto DataWindow.
    public class dw_rubricas
    {
        // Consulta original detectada desde el SRD (puede estar vacía si el objeto es sólo de update)
        public const string Sql = @"SELECT rubricas.rubrica,        rubricas.nombre   FROM rubricas ORDER BY 2";
