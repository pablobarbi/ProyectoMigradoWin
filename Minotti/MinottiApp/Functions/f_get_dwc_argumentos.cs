...
{
    /// <summary>
    /// Migración de PowerBuilder: f_get_dwc_argumentos.srf
    /// Firma original: global function integer f_get_dwc_argumentos (datawindowchild dwc, ref string as_argnames[], ref string as_argdatatypes[])
    /// Mantengo nombre de archivo, clase y método.
    /// </summary>
    public static class f_get_dwc_argumentos
    {
        /// <summary>
        /// Extrae los argumentos definidos en la sintaxis del DataWindowChild (dwc.Syntax ó dwc.Object.DataWindow.Syntax),
        /// buscando bloques tipo 'arg(name=..., type=...)'. Devuelve la cantidad encontrada.
        /// </summary>
        public static int fget_dwc_argumentos(datawindowchild dwc, ref string[] as_argnames, ref string[] as_argdatatypes)
        {
            if (dwc == null) return -1;
            var syntax = dwc.Syntax ?? dwc.Object?.DataWindow?.Syntax ?? string.Empty;
