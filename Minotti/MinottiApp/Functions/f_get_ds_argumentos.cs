...
{
    /// <summary>
    /// Migración de PowerBuilder: f_get_ds_argumentos.srf
    /// Firma original: global function integer f_get_ds_argumentos (DataStore dw, ref string as_argnames[], ref string as_argdatatypes[])
    /// Mantengo nombre de archivo, clase y método.
    /// </summary>
    public static class f_get_ds_argumentos
    {
        /// <summary>
        /// Extrae los argumentos definidos en la sintaxis del DataWindow (dw.Syntax),
        /// buscando bloques tipo 'arg(name=..., type=...)'. Devuelve la cantidad encontrada.
        /// </summary>
        public static int fget_ds_argumentos(datastore dw,out string[] as_argnames,out string[] as_argdatatypes)
        {
            if (dw == null)
            {
            ...
            var syntax = dw.Syntax ?? string.Empty;
