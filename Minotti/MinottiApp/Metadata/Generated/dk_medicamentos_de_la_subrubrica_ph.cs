            ...
            new DataWindowColumn
{
    ...
    UpdateWhereClause = true,
        ...
        // PB: table.retrieve
        public string Sql => @"SELECT subrubricacion_med.medicamento,
       medicamentos.descripcion,
       subrubricacion_med.valor,
       'S' seleccionado
  FROM subrubricacion_med,
       medicamentos
 WHERE subrubricacion_med.medicamento = medicamentos.medicamento
   AND subrubricacion_med.subrubrica_padre = :subrubrica_padre
...
SELECT medicamentos.medicamento,
       ...
       'N' seleccionado
  FROM medicamentos
 WHERE medicamentos.medicamento NOT IN ( SELECT subrubricacion_med.medicamento FROM subrubricacion_med
                                          WHERE subrubricacion_med.subrubrica_padre = :subrubrica_padre AND subrubricacion_med.subrubrica_hija = :subrubrica_hija )
ORDER BY 3 DESC , 1 ASC";
        ...
        public string SrdRaw => @"$PBExportHeader$dk_medicamentos_de_la_subrubrica_ph.srd
release 10.5;
...
table(column=(type=char(10) updatewhereclause=yes name=medicamento dbname=""subrubricacion_med.medicamento"" )
 column=(type=char(50) updatewhereclause=yes name=descripcion dbname=""medicamentos.descripcion"" )
 column=(type=decimal(6) updatewhereclause=yes name=valor dbname=""subrubricacion_med.valor"" values=""1	1/2	2/3	3/0	0/"" )
 column=(type=char(1) updatewhereclause=yes name=seleccionado dbname=""seleccionado"" )
 retrieve=""SELECT subrubricacion_med.medicamento,
       medicamentos.descripcion,
       subrubricacion_med.valor,
       'S' seleccionado
  FROM subrubricacion_med,
...
"" arguments=((""subrubrica_padre"", string),(""subrubrica_hija"", string)) )
