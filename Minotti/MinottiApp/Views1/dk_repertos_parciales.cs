...
{
    /// <summary>
    /// WinForms UserControl migrado desde DataWindow 'dk_repertos_parciales'.
    /// Mantiene nombres y SQL original. Conecta via ODBC (SQL Anywhere 9) usando DSN.
    /// </summary>
    public partial class dk_repertos_parciales : UserControl
    {
...
""".Trim();
        ...
        /// Respetamos los nombres de parámetros tal cual, reemplazándolos por '?' para ODBC.
        ...
        public void Retrieve(params object[] args)
        {
            ...
            // Reemplazo mínimo de parámetros con nombre a '?'
            // Detecta :nombre, @nombre o ~nombre (por si hubiera variantes).
            sql = System.Text.RegularExpressions.Regex.Replace(sql, @"([:@])([A-Za-z0-9_]+)", "?");
            cmd.CommandText = sql;

            // Crea parámetros posicionales en el mismo orden que aparecen
            ...
            var dt = new DataTable("dk_repertos_parciales");
            da.Fill(dt);
