...
namespace Minotti.Repositories
{
    ...
    // Migrado desde PowerBuilder DataWindow: dk_medicamentos_de_la_rubrica.srd
    // Mantiene el nombre original del objeto DataWindow.
    public class dk_medicamentos_de_la_rubrica : datastore
    {
...
WHERE capitulacion_med.medicamento = medicamentos.medicamento
  AND capitulacion_med.capitulo = ?
  AND capitulacion_med.rubrica = ?
UNION
...
WHERE medicamentos.medicamento NOT IN (
          ...
          FROM capitulacion_med
          WHERE capitulacion_med.capitulo = ?
            AND capitulacion_med.rubrica = ?
      )
